        ...
        <div class="resumeSection">
          ...
          <div class="resumeSection__form">
             <div
              class="bottom-action"
              v-if="
                resume.education_list && resume.education_list.length === 0
              "
            >
              <i
                class="el-icon-plus el-icon-plus-top"
                @click="resume.education_list.push({})"
                >添加</i
              >
            </div>
        ...
        <div class="resumeSection">
          ...
          <div class="resumeSection__form resumeSection__form-project">
            <div
              class="bottom-action"
              v-if="
                resume.project_list && resume.project_list.length === 0
              "
            >
              <i
                class="el-icon-plus el-icon-plus-top"
                @click="resume.project_list.push({})"
                >添加</i
              >
            </div>
...
export default {
  ...
  created() {
    ...
    fetchResumeData(countId)
      .then((res) => {
        loading.close();
        this.resume = res.resumeDetail;
      })
      .catch((err) => {
        loading.close();
      });
  ...
  methods: {
    ...
    async submit() {
      try {
        //所有的表单验证 使用了element-ui 自带属性的prop 进行表单验证  需给表单绑定 :rules 传入验证规则
        await Promise.all(
      ...
      payload.id = this.state.userInfo.id;
      console.log("payload", payload);

      this.submitLoading = true;
      await fetchResumeEditData(payload);
